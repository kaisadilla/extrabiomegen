...
import dev.azariadev.extrabiomegen.Utils;
...
public class VoronoiMap {
    ...
    private static final float JITTER = 1f;
    ...
    public VoronoiMap (int seed, int cellSize) {
        _seed = seed;
        _cellSize = cellSize;

        _valueNoise = new FastNoiseLite(Utils.saltSeed(seed, 81082));
        _valueNoise.SetNoiseType(FastNoiseLite.NoiseType.Value);
        _valueNoise.SetFrequency(1f);
    ...
    public void setWarpScale (double warpScale) { _warpScale = warpScale; }
    public void setWarpStrength (double warpStrength) { _warpStrength = warpStrength; }

    public void getCellOrigin (int x, int y, int[] out) {
        double warpX = _warpX.getValue(x * _warpScale, y * _warpScale, 0.0);
        double warpY = _warpZ.getValue(x * _warpScale, y * _warpScale, 0.0);

        x += (warpX * _warpStrength);
        y += (warpY * _warpStrength);

        int xCell = floorDiv(x, _cellSize);
        int yCell = floorDiv(y, _cellSize);

        double minDistSq = Double.MAX_VALUE;
        float xOrigin = 0;
        float yOrigin = 0;

        int xPrime = (xCell - 1) * PRIME_X;
        int yPrimeBase = (yCell - 1) * PRIME_Y;

        for (int dx = xCell - 1; dx <= xCell + 1; dx++) {
            int yPrime = yPrimeBase;

            for (int dy = yCell - 1; dy <= yCell + 1; dy++) {
                // todo: try to make it faster by storing each cell's pos.

                //double px = (dx * _cellSize) + _jitterNoiseX.GetNoise()
                int hash = hash(_seed, xPrime, yPrime);
                int idx = hash & (255 << 1);

                float xCellPoint = ((float)dx + RANDOM_VEC2[idx] * JITTER) * _cellSize;
                float yCellPoint = ((float)dy + RANDOM_VEC2[idx | 1] * JITTER) * _cellSize;
                float xWorld = xCellPoint - x;
                float yWorld = yCellPoint - y;

                float distSq = (xWorld * xWorld) + (yWorld * yWorld);
    ...
    public float getValue (int x, int y) {
        ...
        return _valueNoise.GetNoise(xOrigin, yOrigin);
    }
    ...
    public static final float[] RANDOM_VEC2 = {
